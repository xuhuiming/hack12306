...
class hackTickets(object):
    ...
    def readConfig(self, config_file='config.ini'):
        print("加载配置文件...")
        # 补充文件路径，获得config.ini的绝对路径，默认为主程序当前目录，即cmd中python命令所在目录
    ...
    def confirmSeat(self):
        ...
        if self.driver.find_by_text(u"硬座余票<strong>0</strong>张") == []:#有硬座,确认提交
            ...
            if self.noseat_allow == 0:#不许无座，上一步
                self.driver.find_by_id('back_edit_id').click()
            elif self.noseat_allow == 1:#允许无座，提交订单
                self.driver.find_by_id('qr_submit_id').click()
    ...
    def buyTickets(self):
        ...
        try:
            ...
            # 确认订单
            self.confirmOrder()
            # 提交订单#########无座不能提交，提交了还要取消，一天只有三次机会取消订单
            ###########################self.submitOrder()
            # 确认选座
            ###########################self.confirmSeat()
