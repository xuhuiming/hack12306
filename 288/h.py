...
class hackTickets(object):
    ...
    def buyOrderZero(self):
        ...
        while self.driver.url == self.ticket_url:
            ...
            #sleep(0.05)
            try:
                self.driver.find_by_text(u"查询").click()#网慢可能导致不可点击异常而退出
                count += 1
                print(u"循环点击查询... 第 %s 次" % count)
            except:
                sleep(0.05)
    ...
    def selUser(self):
        ...
        for user in self.users:
            ...
            if user.find('学生')>=0:
                self.driver.find_by_id('dialog_xsertcj_ok').first.click() #确定！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    ...
    def submitOrder(self):#########无座不能提交，提交了还要取消，一天只有三次机会取消订单
        print(u"提交订单...")
        sleep(1)
        self.driver.find_by_id('submitOrder_id').click()#不可点击？

    def confirmSeat(self):
        # 若提交订单异常，请适当加大sleep的时间
        sleep(1)
        print(u"确认选座...")
        print(self.driver.find_by_text(u"硬座余票<strong>0</strong>张"))
        # if self.driver.find_by_text(u"硬座余票\"<strong>0</strong>\"张") == None:#有硬座,确认提交

        # if self.driver.find_by_text(u"<strong>0</strong>") == None:#有硬座,确认提交
        if self.driver.find_by_text(u"硬座余票<strong>0</strong>张") == []:#有硬座,确认提交
            print(u"确认提交")#一直没执行
